...
getPTM <- function (x){
  ...
  ptm <- data.frame(package = tmp$Package, title=tmp$Title, maintainer=tmp$Maintainer)
  return(ptm)
}

# extract maintainers, close to 8k packages take a while ...
...
su[order(su$package),]
# write.csv(su[order(su$package),], "SUMaintainers.csv")
